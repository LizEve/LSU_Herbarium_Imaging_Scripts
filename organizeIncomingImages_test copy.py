...
def moveFiles(sourceFolder,destinationFolder,portalFolders,otherFolders,barcodeMax,barcodeMin,outLogsuffix):
    ...
    for folder in portalFolders:
        ...
        for filename in os.listdir(folderPath):
            ...
            # For files with accepted barcodes 
            if "BadBarcode" not in destinationFilePath:
            	
            	# Try to move path, if there is a problem it will error out and print the file with an issue. 
                try:
                	# If the path does not exist, create the necessary folders
                ...
                # If the above can't be done, an error will be printed to a log file. \
                except:
                    print("File failed to move "+sourceFilePath)

            # If barcode is flagged as bad, move directly to BadBarcode folder and do not write to log. 
            else:
                try:
                    shutil.copy(sourceFilePath,destinationFilePath)
                except:
    ...
    # For other project folders, do not check barcode or file name, move entire folder structure                
    for folder in otherFolders:
	    # Get full folder path 
        folderPath=os.path.join(sourceFolder,folder)
		
		# Get files in any nested folders that exists beyond the organizing folder, for LSU it will be named "Random"
        for filepath in pathlib.Path(folderPath).glob('**/*'):
			sourceFilePath = filepath.absolute()
			# Isolate folder structure within "Random" folder for each file
			nestedFolderPath=str(sourceFilePath).strip(folderPath)
			destinationFilePath=os.path.join(destinationFolder,nestedFolderPath)
			
			# Same making folders and move file as above, but not written to log. 
			try:
				if not os.path.exists(destinationFolderPath):
				    pathlib.Path(destinationFolderPath).mkdir(parents=True)           
				shutil.copy(sourceFilePath,destinationFilePath)
