...
def oldPathDict(roots):
    ...
    #output = open("oldPathDictionary.pkl",'wb')
...
def compareFiles(oldPathDictionary,portalDictionary):
    ...
    # barcode:portal
    barcodeNoImageDict={}
    imageOut = open("barcodeImageDict_hasboth.txt",'w')
    noimageOut = open("imageNoBarcode.txt",'w')
    print(len(oldDictionary_BCcaps),len(portalDictionary_BCcaps))
    # Iterate through barcodes that are in the portal database
    for bcp in oldDictionary_BCcaps:
        # If barcode has image files... 
        if bcp in portalDictionary_BCcaps:
            barcodeImageDict[bcp]=portalDictionary_BCcaps[bcp]
            imageOut.write(str(bcp)+":"+str(barcodeImageDict[bcp])+"\n")
        # If barcode has no image files
        elif bcp not in portalDictionary_BCcaps:
            # keep track of specify records with no image file. barcode:portal
            barcodeNoImageDict[bcp]=oldDictionary_BCcaps[bcp]
            noimageOut.write(str(bcp)+":"+str(oldDictionary_BCcaps[bcp])+"\n")
    imageOut.close()
    noimageOut.close()
...
oldRoots = [rootLSU,rootNO1,rootNO2,rootLSUS]
...
f1 = open('oldPathDictionaryMay30.pkl', 'rb')
oldPathDictionary=pickle.load(f1)
f1.close()
...
f2 = open('portalDictionaryMay30.pkl', 'rb')
portalDictionary=pickle.load(f2)
f2.close()
